...
export function ProsConsTemplate({ data, format, theme, resolution }: { data: ProsConsData; format: FormatType; theme: ThemeType; resolution: ResolutionType }) {
  ...
  return (
    <div style={{ width: size.width, height: size.height }} className={`asset-safe relative overflow-hidden ${t.bg}`}>
      ...
      <div className={`relative flex h-full flex-col ${instagram ? "p-10" : "p-20"}`}>
        <div className={`mb-6 grid gap-6 ${instagram ? "grid-cols-[1fr_260px] items-stretch" : "grid-cols-[1fr_420px] items-end"}`}>
          <div className="min-w-0">
            <p className={`mb-5 w-fit rounded-md ${instagram ? "px-5 py-2 text-2xl" : "px-6 py-3 text-3xl"} font-black uppercase ${t.accent}`}>Pros y contras</p>
            <AutoFitText as="h1" maxSize={instagram ? 62 : vertical ? 92 : 108} minSize={30} lines={instagram ? 3 : 2} lineHeight={0.9} className="max-w-[1320px] text-balance font-black drop-shadow-[0_8px_18px_rgba(0,0,0,.35)]">
        ...
        <div className={`grid min-h-0 flex-1 gap-5 ${vertical || instagram ? "grid-rows-2" : "grid-cols-2"}`}>
        ...
        <div className={`mt-5 rounded-lg border ${instagram ? "p-5" : "p-8"} ${t.panel}`}>
          <p className={`${instagram ? "text-2xl" : "text-3xl"} font-black uppercase ${t.muted}`}>Veredicto</p>
          <AutoFitText as="p" maxSize={instagram ? 32 : 48} minSize={18} lines={instagram ? 2 : 3} lineHeight={1.16} className="mt-2 font-black text-pretty">
  ...
  return (
    <section className={`overflow-hidden rounded-lg border ${panel}`}>
      <div className={`flex items-center gap-5 ${compact ? "px-6 py-5" : "px-8 py-7"} ${headerClass}`}>
        <span className={`grid ${compact ? "size-12" : "size-16"} place-items-center rounded-md bg-black/16`}>
